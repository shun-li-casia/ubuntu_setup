...
if [[ "$USE_SUDO" == "0" ]] ; then
    ...
    XBACKLIGHT_SET="pkexec xfpm-power-backlight-helper"
else
    # XBACKLIGHT_SET="sudo xbacklight"
    XBACKLIGHT_SET="sudo pkexec xfpm-power-backlight-helper"
fi

case $BLOCK_BUTTON in
  # 3) xset dpms force off ;; # right click
  4) $XBACKLIGHT_SET --set-brightness `expr $CURRENT + $STEP_SIZE` ;; # scroll up
  5) $XBACKLIGHT_SET --set-brightness `expr $CURRENT - $STEP_SIZE` ;; # scroll down, decrease
esac
