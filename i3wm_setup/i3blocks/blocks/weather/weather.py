...
FORECAST_CACHE_FILE = os.path.dirname(
    os.path.realpath(__file__)) + "/forecast.xml"

# Emojis associatted with weather        # Day  # Night
WEATHER_TYPES = {
    "Fair": ["☀️", "🌙"], #pylint: disable=C0326
...
def get_xml_root():
    ...
    try:
        ...
        if yr_response.status_code != 200:
            raise RuntimeError('Error: YR status code ' +
                               str(yr_response.status_code))
...
def main():
    ...
    try:
        ...
        sun_rise_time = sun_rise_time[sun_rise_time.find('T') +
                                      1:len(sun_rise_time) - 3]
        sun_set_time = xml_root.find("sun").attrib.get("set")
        sun_set_time = sun_set_time[sun_set_time.find('T') +
                                    1:len(sun_set_time) - 3]
    ...
    if rise_fall_available:
        ...
        is_night = 1 if now.time() < sun_rise.time() or sun_set.time(
        ) < now.time() else 0
    else:
    ...
    if weather in WEATHER_TYPES:
        ...
        print(weather + ": " + WEATHER_TYPES.get(weather)[is_night] + " ",
              end="")
    else:
